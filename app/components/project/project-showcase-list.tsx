...
export default function ProjectShowcaseList(props: ProjectShowcaseListProps) {
  return (
    ...
    >
      <span
        className={`hidden text-6xl font-semibold transition-colors duration-300 lg:block ${
          props.activeProject === props.data.index ? "text-accent" : "text-accent/70"
        }`}
      >
        {props.data.index + 1}.
      </span>
      <span className="text-3xl font-semibold text-accent-foreground transition-colors duration-300 sm:text-4xl md:text-5xl lg:hidden">
        {props.data.index + 1}.
      </span>
      <div className="flex flex-col gap-2 max-w-xl">
        <Link href={props.data.href} className="relative max-w-max">
          <span
            className={`hidden text-6xl font-semibold transition-colors duration-300 lg:block ${
              props.activeProject === props.data.index ? "text-accent-foreground" : "text-accent/70"
            }`}
          >
            {props.data.title}
          </span>
          ...
          <span
            className={`absolute -bottom-1 left-0 hidden h-1 origin-left rounded-lg bg-accent transition-[width] duration-300 group-hover:w-full lg:block ${
